...
from utils import compute_wasserstein, vectorize
...
class SeqVae(nn.Module):
    ...
    def compute_k_step_log_q(self, y):
        ...
        if self.k_step == 1:
            ...
            x_k = vectorize(x_samples[:, 1:], self.k_step)
            mu_k = vectorize(mu[:, 1:], self.k_step)
            var_k_step = vectorize(var[:, 1:], self.k_step)
    ...
    def compute_k_step_log_prior(self, x_samples):
        ...
        if self.k_step == 1:
            ...
            _, mu_k_ahead, var_k_ahead = self.prior.sample_k_step_ahead(x_prev[:, : -(self.k_step+1)].reshape(-1, 1, x_prev.shape[-1]),
                                                                        self.k_step, u=x_k[..., dx:],
                                                                        keep_trajectory=True)
            #
            log_k_step_prior = torch.mean(Normal(mu_k_ahead, torch.sqrt(var_k_ahead)).log_prob(x_k[..., :dx]), -2)
            ...
            for i in torch.arange(self.k_step - 1):
                ...
                _, mu_k, var_k = self.prior.sample_k_step_ahead(x_prev[..., -(K_ahead + 1), :].unsqueeze(1), K_ahead,
                                                                u=x_samples[..., -K_ahead:, dx:], keep_trajectory=True)

                log_prior = Normal(mu_k, torch.sqrt(var_k)).log_prob(x_samples[:, -K_ahead:].reshape(-1, K_ahead, dx))
                log_k_step_prior = log_k_step_prior + torch.sum(log_prior, (-1, -2))/K_ahead

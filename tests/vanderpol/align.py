...
def train_invertible_mapping(ref_vae, train_dataloader, dy_ref,
    ...
    if distribution == 'Normal':
        ...
        f_dec_params = (f_dec_mean, f_dec_var)
        f_dec_params_list = list(f_dec_mean.parameters()) + list(f_dec_var.parameters())

    elif distribution == 'Poisson':

        f_dec_params = f_dec_mean
        f_dec_params_list = list(f_dec_mean.parameters())

    training_losses = []
    opt = torch.optim.AdamW(params=list(f_enc.parameters()) + f_dec_params_list,
    ...
    for _ in tqdm(range(n_epochs)):
        for y, in train_dataloader:
            ...
            loss.backward()
            torch.nn.utils.clip_grad_norm_(list(f_enc.parameters()) + f_dec_params_list,
    ...
    return (f_enc, f_dec_params), training_losses
